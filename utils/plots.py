...
class Annotator:
    ...
    def box_label(self, box, label='', color=(128, 128, 128), txt_color=(255, 255, 255)):
        ...
        if self.pil or not is_ascii(label):
            ...
            cv2.rectangle(self.im, p1, p2, color, thickness=self.lw, lineType=cv2.LINE_AA)  # no modify
            # cv2.rectangle(self.im, p1, p2, color, thickness=max(self.lw-1, 2), lineType=cv2.LINE_AA)  # daaize modify 加粗边框
            if label:
    ...
    def poly_label(self, poly, label='', color=(128, 128, 128), txt_color=(255, 255, 255)):
        if self.pil or not is_ascii(label):
            ...
            if label:
                ...
                cv2.putText(self.im, label, (x_label, y_label + h), 0, self.lw / 6, txt_color, thickness=tf, lineType=cv2.LINE_AA)

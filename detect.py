...
def run(weights=ROOT / 'yolov5s.pt',  # model.pt path(s)
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        for i, det in enumerate(pred):  # per image
            ...
            if len(det):
                ...
                for *poly, conf, cls in reversed(det):
                    ...
                    if save_img or save_crop or view_img:  # Add poly to image
                        ...
                        label = None if hide_labels else (names[c] if hide_conf else f'{names[c]} {conf:.2f}')  # 显示识别class名和置信度
                        # label = None if hide_labels else (names[c] if hide_conf else f'{names[c]}')  # 显示识别class名, 不显示置信度
            ...
            if view_img:
            # if not view_img:  # daaize 改
            #     cv2.putText(im0, f"{n} {names[int(c)]}{'s' * (n > 1)}", (20, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255, 0, 0), 3)  # daaize 单类计数输出
...
def parse_opt():
    ...
    parser.add_argument('--weights', nargs='+', type=str, default=ROOT / 'runs/train/exp1/weights/best.pt', help='model path(s)')
    parser.add_argument('--source', type=str, default='/media/lyt/00F68E2CF68E2250/yolov5_obb-master/dataset/3nd_cleanSpike/test/', help='file/dir/URL/glob, 0 for webcam')
    ...
    parser.add_argument('--line-thickness', default=4, type=int, help='bounding box thickness (pixels)')   # default=2
    parser.add_argument('--hide-labels', default=True, action='store_true', help='hide labels')  # hide detect label

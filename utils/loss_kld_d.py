...
# https://github.com/lx-cly/YOLOv5_OBB_KLD/blob/main/yolov5_obb/utils/loss_kld.py
"""
Loss functions
"""
import math
...
class ThetaL1loss(nn.Module):
    def __init__(self):
        super(ThetaL1loss, self).__init__()
        self.thetaL1loss = nn.L1Loss()

    def forward(self, pbox, tbox):  # pred [[x,y,w,h,angle], ...]

        # 角度制-弧度制
        pre_angle_radian = pbox[:, 4]   # pbox = torch.cat((pxy,pwh,angele), 1)
        target_angle_radian = tbox[:, 4]  # tbox.append(torch.cat((gxy - gij, gwh,theta), 1))

        # thetaL1loss
        thetaL1loss = self.thetaL1loss(pre_angle_radian, target_angle_radian)
    ...
    def __call__(self, p, targets):  # predictions, targets, model
        ...
        for i, pi in enumerate(p):  # layer index, layer predictions
            ...
            if n:
                ...
                ltheta = self.bbox(pbox, tbox[i])  # daaize add

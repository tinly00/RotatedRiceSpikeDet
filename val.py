...
def save_one_txt(predn, save_conf, shape, file):
    ...
    for *xyxy, theta, conf, cls in predn.tolist():
        xywh = (xyxy2xywh(torch.tensor(xyxy).view(1, 4)) / gn).view(-1).tolist()  # normalized xywh
        line = (cls, *xywh, theta, conf) if save_conf else (cls, *xywh, theta)  # label format
...
def run(data,
    ...
    if training:  # called by train.py
        ...
        # Load model
        model = DetectMultiBackend(weights, device=device, dnn=dnn)    # daaize 用到的网络模型，可debug查看
    ...
    for batch_i, (im, targets, paths, shapes) in enumerate(pbar):
        ...
        # print(f"\n\n=======================================\nout:{type(out)}\ntrain_out:{type(train_out)}\nwhatIsIt:{type(whatIsIt)}")
        dt[1] += time_sync() - t2  # 20230701 daaize
        ...
        for si, pred in enumerate(out): # pred (tensor): (n, [cxcylsθ, conf, cls])
            ...
            pred_hbba = torch.cat((hbboxn, pred[:, 4:5], pred_polyn[:, -2:]), dim=1)
            ...
            if nl:
                ...
                if plots:
                    confusion_matrix.process_batch(pred_hbbn, labels_hbbn)  # compute confusion_matrix
            else:
            ...
            if save_txt: # just save hbb pred results!
                ...
                save_one_txt(pred_hbba, save_conf, shape, file=save_dir / 'labels' / (path.stem + '.txt'))
...
def parse_opt():
    ...
    parser.add_argument('--data', type=str, default=ROOT / 'data/obbspike_polydata.yaml', help='dataset.yaml path')
    # parser.add_argument('--weights', nargs='+', type=str, default=ROOT / 'runs/train/yolov5m_finetune/weights/best.pt', help='model.pt path(s)')
    parser.add_argument('--weights', nargs='+', type=str, default=ROOT / 'runs/train/yyds_exp90/weights/best.pt', help='model.pt path(s)')
    ...
    parser.add_argument('--iou-thres', type=float, default=0.2, help='NMS IoU threshold')  # 0.4
    ...
    # parser.add_argument('--save-json', action='dotav15_test_split', help='save a COCO-JSON results file')
    parser.add_argument('--project', default=ROOT / 'runs/val', help='save to project/name')
    parser.add_argument('--name', default='exp', help='save to project/name')
    # parser.add_argument('--name', default='dotav15_test_split', help='save to project/name')
...
# save txt命令: python val.py --weight runs/train/exp57/weights/best.pt --device 0 --save-txt